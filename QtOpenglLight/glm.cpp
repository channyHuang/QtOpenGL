 ...
 */
GLMmodel::GLMgroup* _glmFindGroup (GLMmodel* model, char* name)
{
  GLMmodel::GLMgroup* group;
 ...
 */
GLMmodel::GLMgroup* _glmAddGroup (GLMmodel* model, char* name)
{
  GLMmodel::GLMgroup* group;

  group = _glmFindGroup(model, name);
  if (!group) {
    group = (GLMmodel::GLMgroup*)malloc(sizeof(GLMmodel::GLMgroup));
 ...
 */
char* _glmDirName (const char* path)
{
  char* dir;
  char* s;
...
GLvoid _glmReadMTL (GLMmodel* model, char* name)
{
  ...
  for (i = 0; i < nummaterials; i++) {
	...
	model->materials[i].reflection=0.0f;
	model->materials[i].refraction=0.0f;
	model->materials[i].g=1.0f;
  }
  ...
  while(fscanf(file, "%s", buf) != EOF) {
    ...
    case 'N':
      ...
      model->materials[nummaterials].shininess /= 500.0;
	...
	case 'r':
	fscanf(file, "%f",
	       &model->materials[nummaterials].reflection);
	break;
	case 't':
	fscanf(file, "%f",
	       &model->materials[nummaterials].refraction);
	break;
	case 'g':
	fscanf(file, "%f",
	       &model->materials[nummaterials].g);
	break;
      case 'd':
	fscanf(file, "%f %f %f",
 ...
 */
GLvoid _glmWriteMTL (GLMmodel* model, const char* modelpath, const char* mtllibname)
{
  FILE* file;
  char* dir;
  char* filename;
...
GLvoid _glmFirstPass (GLMmodel* model, FILE* file) 
{
  ...
  GLMmodel::GLMgroup* group;		/* current group */
  unsigned  v, n, t;
  char      buf[128];
...
GLvoid _glmSecondPass (GLMmodel* model, FILE* file)
{
  ...
  GLMmodel::GLMgroup* group;		/* current group pointer */
  ...
  /* on the second pass through file, read all data into allocated arrays */
...
GLMmodel::GLMmodel (const string & filename)
  : pathname(strdup(filename.c_str())),
    mtllibname(NULL),
    numvertices(0),
    vertices(NULL),
    numnormals(0),
 ...
 */
GLvoid GLMmodel::getDimensions (GLfloat* dimensions)
{
  assert(dimensions);
 ...
 * Default winding is counter-clockwise.  Also changes the direction
 * of the normals.
 */ 
GLvoid GLMmodel::reverseWinding ()
{
  GLuint i, swap;
...
/* Generates facet normals for a model (by taking the cross product of
 * the two vectors derived from the sides of each triangle).  Assumes
 * a counter-clockwise winding.
  */
GLvoid GLMmodel::generateFacetNormals ()
{
  GLuint  i;
  GLfloat u[3];
  GLfloat v[3];
...
/* Generates smooth vertex normals for a model.  First builds a list
 * of all the triangles each vertex is in.  Then loops through each
 * vertex in the the list averaging all the facet normals of the
 ...
 * product of a facet normal and the facet normal associated with the
 * first triangle in the list of triangles the current vertex is in is
 * greater than the cosine of the angle parameter to the function,
 * that facet normal is not added into the average normal calculation
 * and the corresponding vertex is given the facet normal.  This tends
 * to preserve hard edges.  The angle to use depends on the model, but
 * 90 degrees is usually a good start.
 *
 * angle - maximum angle (in degrees) to smooth across 
 */
GLvoid GLMmodel::generateVertexNormals (GLfloat angle)
{
  GLMnode*  node;
  GLMnode*  tail;
  GLMnode** members;
  ...
  for (i = 1; i <= numvertices; i++) {
    ...
    if (avg) {
      ...
      normals[3 * numVNormals + 0] = average[0];
      normals[3 * numVNormals + 1] = average[1];
      normals[3 * numVNormals + 2] = average[2];
      avg = numVNormals;
      numVNormals++;
    }
	...
	normals[3 * numVNormals + 0] = 
	  facetnorms[3 * triangles[node->index].findex + 0];
	normals[3 * numVNormals + 1] = 
	  facetnorms[3 * triangles[node->index].findex + 1];
	normals[3 * numVNormals + 2] = 
	  facetnorms[3 * triangles[node->index].findex + 2];
	if (triangles[node->index].vindices[0] == i)
	  triangles[node->index].nindices[0] = numVNormals;
	else if (triangles[node->index].vindices[1] == i)
	  triangles[node->index].nindices[1] = numVNormals;
	else if (triangles[node->index].vindices[2] == i)
	  triangles[node->index].nindices[2] = numVNormals;
	numVNormals++;
      }
      node = node->next;
    }
  ...
  normals = (GLfloat*)malloc(sizeof(GLfloat)* 3* (numnormals+1));
  for (i = 1; i <= numnormals; i++) {
    normals[3 * i + 0] = oldNormals[3 * i + 0];
    normals[3 * i + 1] = oldNormals[3 * i + 1];
    normals[3 * i + 2] = oldNormals[3 * i + 2];
  }
  free(oldNormals);
...
/* Generates texture coordinates according to a linear projection of
 * the texture map.  It generates these by linearly mapping the
 * vertices onto a square.
 */
GLvoid GLMmodel::generateLinearTextureCoordinates ()
{
  GLMmodel::GLMgroup *group;
 ...
 * reflection map texture coordinates.  It generates these by using
 * the normal to calculate where that vertex would map onto a sphere.
 * Since it is impossible to map something flat perfectly onto
 * something spherical, there is distortion at the poles.  This
 * particular implementation causes the poles along the X axis to be
 * distorted.
 */
GLvoid GLMmodel::generateSpheremapTextureCoordinates ()
{
  GLMmodel::GLMgroup* group;
...
/* Reads a model description from a Wavefront .OBJ file.  Returns a
 * pointer to the created object which should be free'd with
 * glmDelete().
 *
 * filename - name of the file containing the Wavefront .OBJ format data.
 */
void GLMmodel::readOBJ (const char * filename)
{
  FILE*     file;
 ...
 */
GLvoid GLMmodel::writeOBJ (const string & filename, GLuint mode)
{
  GLuint    i;
  FILE*     file;
  GLMmodel::GLMgroup* group;
  ...
  if (mode & GLM_MATERIAL && mtllibname) {
    fprintf(file, "\nmtllib %s\n\n", mtllibname);
    _glmWriteMTL(this, filename.c_str(), mtllibname);
  }
  ...
  while(group) {
	...
	fprintf(file, "f %d/%d %d/%d %d/%d\n",
		triangles[group->triangles[i]].vindices[0],
		triangles[group->triangles[i]].findex,
		triangles[group->triangles[i]].vindices[1],
		triangles[group->triangles[i]].findex,
		triangles[group->triangles[i]].vindices[2],
		triangles[group->triangles[i]].findex);
 ...
 *            GLM_FLAT and GLM_SMOOTH should not both be specified.  
 */
GLvoid GLMmodel::draw (GLuint mode)
{
  GLuint i;
  GLMmodel::GLMgroup* group;
...
//	   "using only material mode\n");
  ...
  while (group) {
    ...
    for (i = 0; i < group->numtriangles; i++) {
      if (mode & GLM_FLAT)
	glNormal3fv(&facetnorms[3 * triangles[group->triangles[i]].findex]);
      
      if (mode & GLM_SMOOTH)
	glNormal3fv(&normals[3 * triangles[group->triangles[i]].nindices[0]]);
      if (mode & GLM_TEXTURE)
	glTexCoord2fv(&texcoords[2*triangles[group->triangles[i]].tindices[0]]);
      glVertex3fv(&vertices[3 * triangles[group->triangles[i]].vindices[0]]);
#if 0
      printf("%f %f %f\n", 
	     vertices[3 * triangles[group->triangles[i]].vindices[0] + X],
	     vertices[3 * triangles[group->triangles[i]].vindices[0] + Y],
	     vertices[3 * triangles[group->triangles[i]].vindices[0] + Z]);
#endif
      
      if (mode & GLM_SMOOTH)
	glNormal3fv(&normals[3 * triangles[group->triangles[i]].nindices[1]]);
      if (mode & GLM_TEXTURE)
	glTexCoord2fv(&texcoords[2*triangles[group->triangles[i]].tindices[1]]);
      glVertex3fv(&vertices[3 * triangles[group->triangles[i]].vindices[1]]);
#if 0
      printf("%f %f %f\n", 
	     vertices[3 * triangles[group->triangles[i]].vindices[1] + X],
	     vertices[3 * triangles[group->triangles[i]].vindices[1] + Y],
	     vertices[3 * triangles[group->triangles[i]].vindices[1] + Z]);
#endif
      
      if (mode & GLM_SMOOTH)
	glNormal3fv(&normals[3 * triangles[group->triangles[i]].nindices[2]]);
      if (mode & GLM_TEXTURE)
	glTexCoord2fv(&texcoords[2*triangles[group->triangles[i]].tindices[2]]);
      glVertex3fv(&vertices[3 * triangles[group->triangles[i]].vindices[2]]);
#if 0
      printf("%f %f %f\n", 
	     vertices[3 * triangles[group->triangles[i]].vindices[2] + X],
	     vertices[3 * triangles[group->triangles[i]].vindices[2] + Y],
	     vertices[3 * triangles[group->triangles[i]].vindices[2] + Z]);
#endif
    }
 ...
 *            GLM_FLAT and GLM_SMOOTH should not both be specified.  
 */
GLuint GLMmodel::compile (GLuint mode)
{
  GLuint list;
 ...
 */
GLvoid GLMmodel::weld (GLfloat epsilon)
{
  GLfloat* vectors;
  GLfloat* copies;
  GLuint   numvectors;
  ...
  /* normals */
  if (numnormals) {
  numvectors = numnormals;
  vectors    = normals;
  copies = _glmOptimizeVectors(vectors, &numvectors);

  printf("glmOptimize(): %d redundant normals.\n", 
	 numnormals - numvectors);

  for (i = 0; i < numtriangles; i++) {
    triangles[i].nindices[0] = (GLuint)vectors[3 * triangles[i].nindices[0] + 0];
    triangles[i].nindices[1] = (GLuint)vectors[3 * triangles[i].nindices[1] + 0];
    triangles[i].nindices[2] = (GLuint)vectors[3 * triangles[i].nindices[2] + 0];
  }
  ...
  /* copy the optimized vertices into the actual vertex list */
  for (i = 1; i <= numnormals; i++) {
    normals[3 * i + 0] = copies[3 * i + 0];
    normals[3 * i + 1] = copies[3 * i + 1];
    normals[3 * i + 2] = copies[3 * i + 2];
  }
  ...
  /* texcoords */
  if (numtexcoords) {
  numvectors = numtexcoords;
  vectors    = texcoords;
  copies = _glmOptimizeVectors(vectors, &numvectors);

  printf("glmOptimize(): %d redundant texcoords.\n", 
	 numtexcoords - numvectors);

  for (i = 0; i < numtriangles; i++) {
    for (j = 0; j < 3; j++) {
      triangles[i].tindices[j] = (GLuint)vectors[3 * triangles[i].tindices[j] + 0];
    }
  }
  ...
  /* copy the optimized vertices into the actual vertex list */
  for (i = 1; i <= numtexcoords; i++) {
    texcoords[2 * i + 0] = copies[2 * i + 0];
    texcoords[2 * i + 1] = copies[2 * i + 1];
  }
